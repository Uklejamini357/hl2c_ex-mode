...
function PANEL:ApplySchemeSettings()
	self.nameLabel:SetFont("hl2ce_font_sb")
	self.scoreLabel:SetFont("hl2ce_font_sb")
	self.deathsLabel:SetFont("hl2ce_font_sb")
	self.pingLabel:SetFont("hl2ce_font_sb")
end
...
function PANEL:Init()
	...
	self.deathsLabel:SetTextColor(color)
...
function PANEL:UpdatePlayerList()
	for ply, row in pairs(self.playerRows) do
		if !IsValid(ply) or ply:Team() != self.teamToList then
			row:Remove()
