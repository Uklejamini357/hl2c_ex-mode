...
function hl2cAcceptInput(ent, input, activator, caller, value)
	if !game.SinglePlayer() and ent:GetName() == "alyx_briefingroom_exitdoor" and string.lower(input) == "lock" then
		return true
	end

	if !game.SinglePlayer() and ent:GetName() == "barricade_gate" and string.lower(input) == "setanimation" and string.lower(tostring(value)) == "close" then
		return true
	end
end
