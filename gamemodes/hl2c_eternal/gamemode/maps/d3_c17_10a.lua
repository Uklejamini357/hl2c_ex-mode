...
function hl2cAcceptInput(ent, input)
	if !game.SinglePlayer() and ent:GetName() == "first_turret_relay" and string.lower(input) == "trigger" then
		ents.FindByName("barney")[1]:SetLastPosition(Vector(-3306, 7036, 128))
	...
	if ent:GetName() == "barney_nexusahead_lcs_relay" && string.lower(input) == "trigger" then
		...
		ents.FindByName("barney")[1]:SetLastPosition(Vector(-2580, 6482, 512))
		ents.FindByName("barney")[1]:SetSchedule(SCHED_FORCED_GO_RUN)
	end

	if !game.SinglePlayer() and ent:GetName() == "detected_relay" and string.lower(input) == "trigger" then
		ents.FindByName("barney")[1]:SetLastPosition(Vector(-2842, 8016, 128))
		ents.FindByName("barney")[1]:SetSchedule(SCHED_FORCED_GO_RUN)
	end
end
