...
if CLIENT then
	...
	net.Receive("hl2ce_map_event", function(len)
		...
		elseif t == "breen_introduction_01" then
			local tbl = {}
			local s = "Chapter 13"
			for i=1,#s do
				table.insert(tbl, Color(math.random(200,255), math.random(200,255), math.random(200,255)))
				table.insert(tbl, s[i])
			end
			chat.AddText(unpack(tbl))
		elseif t == "breen_introduction_02" then
			local tbl = {}
			local s = "Thy end is near... Thy end is now! Or is it..."
			for i=1,#s do
				table.insert(tbl, Color(math.random(255), math.random(255), math.random(255)))
				table.insert(tbl, s[i])
			end
			chat.AddText(unpack(tbl))
		end
...
function hl2cPlayerSpawn(ply)
	if !game.SinglePlayer() then
		...
		if IsValid(ents.FindByName("pod")[1]) then
...
function hl2cMapEdit()
	...
	if !game.SinglePlayer() then
		...
		viewcontrol:SetKeyValue("fov", "105")
		viewcontrol:Spawn()
...
function hl2cAcceptInput( ent, input, activator, caller, value )
	...
	if !game.SinglePlayer() then
		...
		if ent:GetName() == "logic_fade_view" and string.lower(input) == "trigger" then
		...
		if ent:GetName() == "fade_breen_wins" and input:lower() == "reload" then
			PrintMessage(3, "wow congrats you broke the map")
			return true
		end

		-- prevent the map from breaking
		if input:lower() == "scriptplayerdeath" then
			return true
		end
	end
	...
	if GAMEMODE.EXMode then
		...
		if ent:GetName() == "lcs_BreenOffice02" and input:lower() == "start" then
			timer.Simple(2.5, function()
				...
				net.WriteString("breen_introduction_01")
				net.Broadcast()
			end)
			timer.Simple(4.8, function()
				if !IsValid(ent) then return end
				net.Start("hl2ce_map_event")
				net.WriteString("breen_introduction_02")
				net.Broadcast()
			end)
		end
