...
function hl2cAcceptInput(ent, input, activator)
	...
	if !game.SinglePlayer() and ent:GetName() == "briefing_relay" and input:lower() == "trigger" then
		ALLOWED_VEHICLE = nil
	...
	if !game.SinglePlayer() and ent:GetName() == "lighthouse_secret_door" and string.lower(input) == "close" then
		return true
	end
end
