...
function hl2cAcceptInput(ent, input, activator, caller)
	if !game.SinglePlayer() and ent:GetName() == "lcs_guncave_briefing1" and input:lower() == "start" then
		ALLOWED_VEHICLE = nil
	...
	if !game.SinglePlayer() and ent:GetName() == "relay_guncave_startgunmount" and input:lower() == "enablerefire" then
		ALLOWED_VEHICLE = "Airboat Gun"
		PrintMessage(HUD_PRINTTALK, "You're now allowed to spawn the Airboat & Gun (F3).")
	end

	if !game.SinglePlayer() and ent:GetName() == "door_guncave_entrance" and string.lower(input) == "close" then
		return true
	end
