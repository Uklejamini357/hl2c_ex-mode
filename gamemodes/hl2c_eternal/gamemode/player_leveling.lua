local function canlevel(self)
    if self.IsLevelingup then return false end
    if infmath.ConvertInfNumberToNormalNumber(self.Level) >= self:GetMaxLevel() then
        ...
        return false
    elseif self.XP >= GAMEMODE:GetReqXP(self) and self:CanLevelup() then
        return true
    end
end
...
function meta:GainLevel()
    ...
    if self:HasPerkActive("2_skills_improver") then
        local equalspuse = infmath.ConvertInfNumberToNormalNumber(infmath.floor(self.StatPoints / table.Count(GAMEMODE.SkillsInfo)))
    ...
    if not self:HasEternityUnlocked() and not canlevel(self) then
...
function meta:GainCelestiality()
    if infmath.ConvertInfNumberToNormalNumber(self.Celestiality) >= self:GetMaxCelestiality() then
